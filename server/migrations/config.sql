...
-- 通用配置系统：分组 (config_groups) + 项 (config_items)
-- 版本: PostgreSQL 14+
--
...
CREATE UNIQUE INDEX IF NOT EXISTS uk_config_groups_code ON config_groups (tenant_id, code) WHERE is_deleted = FALSE;
CREATE INDEX IF NOT EXISTS idx_config_groups_tenant ON config_groups (tenant_id) WHERE is_deleted = FALSE;
...
CREATE UNIQUE INDEX IF NOT EXISTS uk_config_items_key ON config_items (tenant_id, group_id, key) WHERE is_deleted = FALSE;
CREATE INDEX IF NOT EXISTS idx_config_items_group ON config_items (group_id) WHERE is_deleted = FALSE;
...
DROP POLICY IF EXISTS tenant_isolation_policy ON config_groups;
CREATE POLICY tenant_isolation_policy ON config_groups
USING (
