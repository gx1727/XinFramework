...
--
-- 内容（按种子概念分组）：
--   1. 平台账号 + 路径 B 兼容性（admin 账号 + super_admin 角色 + admin 借用清理）
--   2. bootstrap 租户
--   3. admin 用户 / 角色 / 角色绑定
--   4. 基础菜单（dashboard / system 及其子菜单）
--   5. 资源（resource / dict / 超级通配）
--   6. admin 角色绑菜单 + 通配资源
--   7. 字典（平台级 gender / user_status / education + bootstrap 副本）
--   8. Config 模块（4 个分组 + 19 个项 + 菜单 + 资源）
--   9. 平台域菜单（tenants / config / frames / avatars）
--  10. flag 资源 seed
...
-- 1. 平台账号 + 路径 B 兼容
-- ============================================
-- 历史：路径 A 把 admin 用户借用在 tenant_id=1（bootstrap）的 users 表里当系统管理员。
-- 路径 B 修正：admin 不再借住租户 users 表；改用 account_roles.role='super_admin'
-- 标识平台身份。下文 admin 借用清理步骤是路径 B 的必备条件。
--
...
-- 1.2 account_roles.role 加 CHECK 约束（路径 B 终态，防止拼写错误）
-- 历史：先 DROP 再 ADD，幂等安全。
...
-- 1.3 给种子 admin 账号加 super_admin 平台角色
INSERT INTO account_roles (account_id, role)
SELECT id, 'super_admin'
FROM accounts
WHERE phone = '13800138000' AND is_deleted = FALSE
ON CONFLICT (account_id, role) DO NOTHING;

-- 1.4 users 表 unique 索引从 (account_id) 改为 (account_id, tenant_id)
-- 允许一个账号在多个租户各有一条 users 记录（每个租户一个身份）。
...
-- 1.5 清掉 admin 账号借用 bootstrap 租户的 users 记录（路径 B 不再需要）
-- 安全检查：如果 admin 账号有 > 1 条 users 记录，说明是合法多租户身份，停止迁移。
DO $$
DECLARE
    cnt INT;
...
BEGIN
    ...
    IF admin_account_id IS NULL THEN
        RAISE NOTICE 'admin 账号 (phone=13800138000) 不存在，跳过清理';
        RETURN;
    END IF;
    SELECT COUNT(*) INTO cnt FROM users WHERE account_id = admin_account_id;
    IF cnt > 1 THEN
        RAISE EXCEPTION 'admin 账号有 % 条 users 记录（预期 0~1）。需人工确认后再清。', cnt;
    END IF;
    DELETE FROM user_roles WHERE user_id IN (SELECT id FROM users WHERE account_id = admin_account_id);
...
-- 2. bootstrap 租户
-- ============================================
-- 单一系统租户：既是 admin 用户在租户域的居住地（admin 用户从这里借住过，
-- 路径 B 下 admin 不再借住，但 bootstrap 仍是"新租户克隆源"——first_install.go
-- 从这里复制 menus / resources / dicts / config_*）。
...
-- admin 用户（路径 B 下 admin 仍可在 bootstrap 租户里作为"初始租户管理员"存在）
...
-- 用户-角色：admin → admin 角色
INSERT INTO user_roles (tenant_id, user_id, role_id)
VALUES (1, 1, 1);

-- ============================================
-- 4. 基础菜单（框架 + system 子菜单）
-- ============================================
...
-- ============================================
-- 7.1 平台级字典（tenant_id=0, scope='platform'）
...
-- 9. 平台域菜单（tenants / config / frames / avatars）
-- ============================================
-- 这些菜单在 bootstrap 租户里（scope='tenant'），是 admin 登录后能看到的"平台管理"入口。
...
SELECT setval('menus_id_seq', GREATEST((SELECT COALESCE(MAX(id), 0) FROM menus), 1000), true);
...
-- 10.1 tenant 域资源：tenant:* 与 config:*
INSERT INTO resources (tenant_id, menu_id, code, name, action, description, sort, status)
SELECT 1,
...
-- 11. 平台域 sys_* 种子
-- ============================================
-- Phase 0023+ 终态：admin 账号对应 sys_users 里的 admin 平台身份 + super_admin 角色。
-- 现在 sys_users 和 sys_user_roles 已建好（init_schema.sql），
...
-- 11.3 admin 绑定 super_admin 平台角色
