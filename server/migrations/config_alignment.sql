...
-- 与 dict.sql 的设计对齐：
--   - config_groups 加 scope / visibility 字段
...
-- 1. config_groups 加 scope / visibility
...
(
    ...
    CONSTRAINT uk_config_visibility UNIQUE (group_id, tenant_id)
);
CREATE INDEX IF NOT EXISTS idx_config_visibility_tenant ON config_visibility (tenant_id);
CREATE INDEX IF NOT EXISTS idx_config_visibility_group  ON config_visibility (group_id);

-- 4. 替换 config_groups 唯一索引为 platform/tenant 分层
...
CREATE UNIQUE INDEX IF NOT EXISTS uk_config_group_code_platform
    ON config_groups (code) WHERE scope = 'platform' AND is_deleted = FALSE;
CREATE UNIQUE INDEX IF NOT EXISTS uk_config_group_code_tenant
    ON config_groups (tenant_id, code) WHERE scope = 'tenant' AND is_deleted = FALSE;
...
-- 6. config_items 按 platform/tenant 分层的 (group_id, key) 唯一
-- 旧 uk_config_item_key 索引可能只按 (group_id, key)
