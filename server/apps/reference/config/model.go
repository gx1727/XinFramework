...
//
// 与 dict 不同的语义：
//   - config_items 用 (group_id, key) 而不是 (dict_id, code) —— 因为配置以 key 而非 code 寻址
...
//
// 与 dict_visibility 同构，仅主键字段名不同（group_id vs dict_id）。
...
// GroupWithItems 分组 + 该分组下所有项（业务合并消费用）
...
type ConfigRepository interface {
	...
	ListPlatformGroups(ctx context.Context) ([]ConfigGroup, error)
	...
	GetPlatformGroupByCode(ctx context.Context, code string) (*ConfigGroup, error)
	CreateGroup(ctx context.Context, tenantID uint, scope string, req CreateGroupRepoReq) (*ConfigGroup, error)
	...
	ListPlatformItemsByGroup(ctx context.Context, groupID uint) ([]ConfigItem, error)
	...
	ListVisibility(ctx context.Context, groupID uint) ([]ConfigVisibility, error)
	UpsertVisibility(ctx context.Context, groupID, tenantID uint, access string) (*ConfigVisibility, error)
	DeleteVisibility(ctx context.Context, groupID, tenantID uint) error
