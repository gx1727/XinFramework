...
// Tree 返回当前 sys 用户可见的菜单树。
//
// 鉴权层级：
//   - 路由层 RequireAnySysRole 已保证 SysRoles 非空（"需要 sys 级角色"）
...
// UserID / SysRoles 注入了 request context）。
func (h *Handler) Tree(c *gin.Context) {
	xc := xincontext.New(c)
	accountID := xc.UserID // sys 用户 JWT UserID 即 account_id
	roles := xc.SysRoles   // 已被中间件校验非空
