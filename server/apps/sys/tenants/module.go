...
// Module returns the sys_tenant module as a BaseModule.
//
// 模块名约定："sys_tenant"（管 tenants 表的 CRUD；与 sys_menu 同属 sys 域）
// 圀cfg.Module: 里以 "sys_tenant" 标识，且属于 alwaysOn 列表（启动必需）
//
// 本模块的双重职责：
//  1. 提供 super_admin 用的租户 CRUD API（路由挂 /tenants：
