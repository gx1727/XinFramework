...
// 注意：必须在 RunInSysTx 调用本方法，否则会被 users 表的 RLS 拦截（current_setting('app.bypass_rls') != 'on'）
...
//
// 注意：必须在 RunInSysTx 内调用，绕过 tenant_users / tenant_user_roles / tenant_roles 的 RLS。
//
