...
// firstInstall 圀RunInSysTx 内执行：root org / admin role / 复制模板菜单资源字典 / admin user / tenant_user_seq
//
...
func firstInstall(ctx context.Context, pool *pgxpool.Pool, tenantID uint, adminAccountID uint) (*FirstInstallReport, error) {
	...
	// 2) 复制 menus（两步法：
	// 2a) 根菜单（parent_id=0）：先入主，拿到新的 code ↀid 映射
	if _, err := q.Exec(ctx, `
	...
	// 2b) 子菜单：甀code 重映尀parent_id 到新租户的同 code 菜单
	if _, err := q.Exec(ctx, `
		INSERT INTO tenant_menus (tenant_id, code, name, subtitle, url, path, icon, sort, parent_id, ancestors, visible, enabled)
	...
	// 3) 复制 resources：用 code 重映尀menu_id
	...
	// 4a) admin role 绑定所有模板菜单（role_menus 全绑定，确保租户冀admin 可访问所有菜单）
	if _, err := q.Exec(ctx, `
	...
	// 6) 复制 dict_items：用 code 重映尀dict_id
	if _, err := q.Exec(ctx, `
	...
	// 6b) 复制 config_items（用 code 重映尀category_id，value 继承 default_value：
	if _, err := q.Exec(ctx, `
	...
	// 7) admin user（仅彀adminAccountID > 0：
	if adminAccountID > 0 {
...
func installAdminUser(ctx context.Context, q db.Querier, tenantID, adminAccountID, rootOrgID, adminRoleID uint, rep *FirstInstallReport) error {
	...
	if accountStatus != 1 {
		rep.WarnMessages = append(rep.WarnMessages,
			fmt.Sprintf("admin_account_id=%d status=%d，跳迀admin user 创建", adminAccountID, accountStatus))
		return nil
	}
