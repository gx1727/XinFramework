// Package sysuser 实现"sys 域用户身份"管理 API（sys_users 表）。
//
// 与 apps/sys/menu 同属 sys 域。关键不变量：
//
//	| 维度          | apps/tenant/user (rbac)       | apps/sys/user              |
//	| tenant_id     | 来自 ctx.TenantID             | 不存在（sys 域无 tenant）   |
//	| DB 事务上下文 | RunInTenantTx                 | RunInSysTx (bypass)        |
//	| 中间件        | Require(permission.P(...))    | RequireSysRole("super_admin") |
//	| 路由前缀      | /api/v1/users                 | /api/v1/sys-users          |
...
//	          ↑ embedded by
//	framework/pkg/sysauth.User             // sys 域 User
//	          ↑ used by
//	apps/sys/user.User                     // 本模块内部 Go struct
//
...
// User 是本模块的内部 Go struct，包装 sysauth.User。
// 字段集与 sysauth.User 完全相同（identity.User 子集）。
// 保持本地 struct 的原因：未来加 sys 专属字段时（sys_level、scope_tags
...
//
// 所有方法必须在 db.RunInSysTx 上下文中调用（不依赖 caller，
// 但 sysauth contract 不强制 —— 是约定的"不变量"）。
