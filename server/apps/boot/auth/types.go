...
// LoginScope 登录作用域：tenant（业务租户登录） / sys（平台管理员登录）
//
// 决定 token 中 tenant_id 含义与后续能访问的路由空间：
//   - tenant → tenant_id > 0，可访问 /api/v1/* 业务域（Auth + RequireTenantContext）
//   - sys    → tenant_id = 0，可访问 /api/v1/sys/* sys 域（Auth + RequireSysRole）
type LoginScope string

const (
	LoginScopeTenant LoginScope = "tenant"
	LoginScopeSys    LoginScope = "sys"
)

// sys 登录专用错误（独立错误码段，不和 tenant 登录冲突）
var (
	ErrSysLoginAccountNotFound = resp.Err(1012, "平台账号不存在")
	ErrSysLoginNotAdmin        = resp.Err(1013, "该账号不具备平台管理员权限")
	ErrSysLoginDisabled        = resp.Err(1014, "平台账号已禁用")
)

// 登录前置检查专用错误
var (
	ErrNoLoginIdentity = resp.Err(1015, "账号无可用登录身份（无 tenant 身份且无 sys 角色）")
)

// Refresh 切租户专用错误
var (
	ErrCrossTenantSwitchFromSys = resp.Err(1016, "平台 token 不能切换到租户，请用 sys-login 重新登录")
)
...
//
// 单身份账号可以跳过此步直接调 /auth/tenant-login 或 /auth/sys-login。
...
//   - sys_available: true 时前端可选择调 /auth/sys-login（不走 tenant）
//   - tenant_identities 为空 + sys_available 为 false：账号无登录权限（403）
//   - tenant_identities 为空 + sys_available 为 true：纯平台账号，precheck 后调 sys-login
...
type loginPrecheckResult struct {
	...
	SysRoleCodes     []string                 `json:"sys_role_codes,omitempty"`
	TenantIdentities []pkgauth.TenantIdentity `json:"tenant_identities"`
}

// sysLoginRequest sys 域登录（super_admin 等）
//
// 不带 tenant_id：平台管理员不属于任何租户；登录后整个会话不出现 tenant 概念。
// 后端验证账号 + 密码 + 至少有一个 sys_role（如 super_admin）。
...
//   - scope=sys    → TenantID = 0，Role 为空（平台管理员无租户角色），SysRoleCodes 必有值
type User struct {
	ID       uint   `json:"id"`
	...
	// SysRoleCodes sys 级角色码列表（super_admin 等），与 JWT claims 对齐，
	// 让前端能在不解析 JWT 的前提下判断是否能访问 /admin/* sys 域路由。
	SysRoleCodes []string `json:"sys_role_codes,omitempty"`
...
type refreshRequest struct {
	...
	//   - 不允许从 sys token 切到租户（sys token 没有 users 上下文）
