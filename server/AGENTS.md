...
├── apps/                                 # 业务模块
│   ├── boot/auth/                        # 登录 / 账号（必装）
│   ├── sys/tenants/                      # sys 域租户 CRUD（必装）
│   ├── sys/user|role|menu|permission/    # sys 管理域
...
└── framework/                            # 框架内核（公开 + internal 两层）
    ...
    │   ├── db/                           # pool Init + Querier + RunInTx/TenantTx/SysTx
    ...
    │   ├── auth/                         # AccountRepository 公开契约
        ...
        │   ├── sysauth/                      # sys 域 User/Role/Menu/Permission 公开契约
        │   ├── tenant/auth/                  # 租户域 User/Role/Org/RoleResource 公开契约
        │   ├── middleware/                   # Require / RequireSysRole / RequireTenantContext
    │   ├── audit/                        # db_logs 写入入口
    │   ├── migrate/                      # 启动期 SQL 迁移
    │   ├── resp/                         # 统一响应 + 错误码分段
    │   ├── logger/                       # zap 包装
...
| `protected` | `/api/v1/sys/*` | `Auth` | sys 域，模块内部追加 `RequireSysRole` |
...
| 域 | 标识 | RLS | API 守卫 |
|---|---|---|---|
| **sys 域** `sys_*` | 无 `tenant_id` | ❌ 不启用 | `RequireSysRole("super_admin")` + `db.RunInSysTx` |
...
| 域 | 表 | 业务模块 |
|---|---|---|
| sys 域 | `sys_users` / `sys_orgs` / `sys_roles` / `sys_menus` / `sys_permissions` / `sys_user_roles` / `sys_role_menus` / `sys_role_permissions` | `apps/sys/*` |
...
| 共享 | `tenants` / `accounts` / `account_auths` / `auth_sessions` | `apps/sys/tenants` + `apps/boot/auth` |
...
| 档 | 模块 | cfg.module 写法 |
|---|---|---|
| `alwaysOn`（3 个，必装） | `system` / `auth` / `sys_tenant` | 不可关 |
| `optOut`（13 个，默认全开） | RBAC（menu/user/role/resource/organization/permission）+ dict/asset/config + sys 管理（sys_user/sys_role/sys_menu/sys_permission） | 框架默认启用，不写即为开 |
...
签发：`GenerateWithSysRoles`（带 sys roles）。`HasSysRole(role)` 便捷判断。模拟登录用 `GenerateImpersonation`。
...
**短路**：拥有 `*:*` 资源权限或 `super_admin` sys 角色时，`Require*` 全部放行（`requireWithSpecs` L119）。
...
- `RunInSysTx(ctx, pool, fn)` — 设 `app.bypass_rls = 'on'` + `app.tenant_id = 0`（sys 域跨租户访问）
...
通配符 `*:*` = 全资源全操作（admin role 默认绑定）。`requireWithSpecs` 在 `Context.HasSysRole(super_admin)` 时短路放行。
...
| 路径 | 模块 |
|---|---|
| `POST /auth/tenant-login` / `sys-login` / `login-precheck` / `select-tenant` / `refresh` / `register` / `login`（兼容） / `platform-login`（兼容转发到 sys-login） | `boot/auth` |
| `GET /health` | `system` |
...
### 11.3 sys 域（super_admin）

| 路径 | 模块 |
|---|---|
| `/sys/tenants`（CRUD + `:id/status` + `:id/purge` + `:id/impersonate`） | `sys/tenants` |
| `/sys/sys-users`（CRUD + `:id/status` + `:id/roles`） | `sys/user` |
| `/sys/sys-roles`（CRUD + `:id/menus` + `:id/permissions`） | `sys/role` |
| `/sys/menus`（CRUD + `/tree`） | `sys/menu` |
| `/sys/sys-permissions`（CRUD） | `sys/permission` |
| `/sys/dicts` + `/sys/dicts/:id/items` + `/sys/dicts/:id/visibility` | `reference/dict` |
| `/sys/configs` + `/sys/configs/:id/items` + `/sys/configs/:id/visibility` | `reference/config` |
| `/sys/system/clear-cache` + `/sys/system/cache/*` | `system` |
...
| sys 域租户 | `server/apps/sys/tenants/` |
| sys 管理域 | `server/apps/sys/{user,role,menu,permission}/` |
