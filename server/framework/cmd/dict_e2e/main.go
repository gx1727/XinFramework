...
func main() {
	...
	if err != nil {
		fmt.Println("insert dict:", err)
		os.Exit(1)
	}
	var tenant1GenderID int
	if err := pgPool.QueryRow(ctx, "SELECT id FROM dicts WHERE tenant_id=1 AND code='gender' AND is_deleted=FALSE").Scan(&tenant1GenderID); err != nil {
		fmt.Println(err)
		os.Exit(1)
	}
	...
	err = db.RunInTenantTx(ctx, pool, 2, func(ctx context.Context) error {
		...
		_ = err
		if d == nil {
			fmt.Println("  no platform gender")
			return nil
		}
