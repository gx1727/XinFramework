...
>
> 文档版本：2026-06（platform_menu / platform_tenant 模块化后无 breaking change）
...
> **复用原则**：`platform_menu` 和 `menu` 共用 `ResMenu`；`platform_tenant` 和未来业务层 `tenant` 共用 `ResTenant`。资源码代表"操作这种资源的权限"，与具体路径无关。
...
平台管理模块（`/api/v1/admin/*`）都强制 `RequirePlatformRole("super_admin")`：
...
[Auth/OptionalAuth]
   ...
   ↓
[RequirePlatformRole] (可选, 平台管理路由组)
