...
> 当前�?**16 �?module**。按 `cfg.Module` 行为�?3 类：3 �?alwaysOn�? �?optOut�? �?optional�?>
> 文档版本�?026-06（config 重构 + platform_menu/platform_tenant 模块化后�?
## 总览
...
**数据�?*：`roles` / `user_roles` / `role_menus` / `role_resources`�?
---

## menu

**职责**：租户内菜单�?CRUD（平台菜单由 `platform_menu` 模块管理）�?
**路由**�?
| Method | Path | Spec | Handler |
|---|---|---|---|
...
**路由**（全部位�?`/api/v1/platform/menus`，group �?`RequirePlatformRole("super_admin")`）：

| Method | Path | Handler |
|---|---|---|
| GET | `/platform/menus` | `List` |
| GET | `/platform/menus/tree` | `Tree` |
| GET | `/platform/menus/:id` | `Get` |
| POST | `/platform/menus` | `Create` |
| PUT | `/platform/menus/:id` | `Update` |
| DELETE | `/platform/menus/:id` | `Delete` |

**数据�?*：`menus`（用 `WHERE tenant_id = 0` 过滤；强�?`platformTenantID = 0`）�?
**关键约定**�?
- 平台菜单与租户菜单共�?`menus` 表，�?`tenant_id` 区分（`0` = 平台�?- 所有写操作�?`db.RunInPlatformTx` 跳过 RLS
- 模板参�?[`apps/platform/menu/`](apps/platform/menu/)（Phase 0021 后新加的命名约定样板�?
---
...
**职责**�?*示例 CMS**，展�?extapi 模式（cms module 自己不连 RBAC 表，而是通过 `extapi.UserFacade` / `TenantFacade` �?module 查询）�?
**路由**�?
| Method | Path | Auth | Handler |
|---|---|---|---|
...
**设计意图**：展示外�?module 如何只通过 `extapi.Provider` 调用平台能力（查用户、查租户），而不直接 import `apps/rbac/user` 等模块�?
---
