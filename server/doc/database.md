...
> 当前 30+ 张表。核心表在 `migrations/framework.sql`，业务表分散在 `cms.sql` / `dict.sql` / `flag.sql` / `asset.sql` / `config.sql`。
...
| `config_groups` | 配置分组 | `code`、`name` |
| `config_items` | 配置项 | `group_id`、`key`、`value JSONB`、`default_value JSONB`、`options JSONB`、`validation JSONB`、`is_public` |
...
**为什么用 `txCtx`？** 因为 `db.GetQuerier(ctx)` 优先返回 `ctx` 上的 tx，整个回调共享一个事务。
...
valueJSON := toJSON(req.Value)        // []byte 或 nil
...
`tenants.config` 是 JSONB 但目前**没有 Go 代码写入**（仅文档预留）。如果将来要写入，记得照 §9.2 加 cast。
