...
export const dictApi = {
  list: () =>
    api<PageResponse<DictItem>>("/dicts"),

  get: (code: string) =>
    api<DictItem>(`/dicts/${code}`),

  create: (data: { code: string; name: string; extend?: Record<string, unknown> }) =>
    api<DictItem>("/dicts", {
      method: "POST",
      body: JSON.stringify(data),
    }),

  update: (id: number, data: { name?: string; extend?: Record<string, unknown> }) =>
    api(`/dicts/${id}`, {
      method: "PUT",
      body: JSON.stringify(data),
    }),
  ...
  createItem: (code: string, data: { code: string; name: string; sort?: number; extend?: Record<string, unknown> }) =>
    api(`/dicts/${code}/items`, {
      method: "POST",
      body: JSON.stringify(data),
    }),

  updateItem: (code: string, itemId: number, data: { name?: string; sort?: number; extend?: Record<string, unknown> }) =>
    api(`/dicts/${code}/items/${itemId}`, {
      method: "PUT",
      body: JSON.stringify(data),
    }),
