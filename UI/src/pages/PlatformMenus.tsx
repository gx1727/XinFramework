...
import { platformMenuApi, type PlatformMenuItem, ApiError } from "@/api"
 ...
 * 所以非 super_admin 的平台用户只能看到被分配的那部分菜单。
 */
...
export function PlatformMenusPage() {
      ...
      const msg =
        err instanceof ApiError
          ...
          : err instanceof Error
            ? err.message
            : "平台菜单加载失败"
      console.error("[PlatformMenus] fetch failed:", err)
      setMenus([])
      setError(msg)
    } finally {
      setLoading(false)
    ...
    () => ({
      items: [
        ...
        },
        {
          ...
          type: "text",
          required: true,
          placeholder: "请输入菜单代码，如 platform-users",
        },
        {
          field: "path",
          ...
          type: "text",
          required: true,
          placeholder: "请输入路由路径，如 /platform/users",
        },
        {
          field: "icon",
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      console.error("[PlatformMenus] delete failed:", err)
      alert("删除失败，请重试")
    } finally {
  ...
  const handleSubmit = async (values: Record<string, unknown>) => {
    ...
    try {
      ...
      console.error("[PlatformMenus] save failed:", err)
      alert("保存失败，请重试")
    } finally {
      setIsSubmitting(false)
    }
    ...
    <PageLayout>
      <div className="px-4 lg:px-6">
        <div className="mb-6 flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold">平台菜单管理</h1>
...
interface PlatformMenuTreeRowProps {
  item: TreeMenuItem
  level: number
