...
const ITEM_TYPE_OPTIONS: { value: ConfigItemType; label: string }[] = [
  { value: "string", label: t.pages.platformConfigs?.typeString || "字符串" },
  { value: "number", label: t.pages.platformConfigs?.typeNumber || "数字" },
  { value: "boolean", label: t.pages.platformConfigs?.typeBoolean || "布尔" },
  { value: "json", label: t.pages.platformConfigs?.typeJson || "JSON" },
  { value: "image", label: t.pages.platformConfigs?.typeImage || "图片" },
  { value: "color", label: t.pages.platformConfigs?.typeColor || "颜色" },
  { value: "select", label: t.pages.platformConfigs?.typeSelect || "单选" },
  { value: "multiselect", label: t.pages.platformConfigs?.typeMultiselect || "多选" },
  { value: "text", label: t.pages.platformConfigs?.typeText || "长文本" },
  { value: "password", label: t.pages.platformConfigs?.typePassword || "密码" },
]
...
export function PlatformConfigsPage() {
  ...
  const [groupToDelete, setGroupToDelete] = useState<ConfigCategory | null>(null)
  ...
  const handleDeleteGroup = async () => {
    ...
    try {
      ...
      const msg = e instanceof Error ? e.message : String(e)
      console.warn("[delete platform group] failed:", e)
      setError(`删除分组失败：${msg}`)
  ...
  const groupFormSchema: FormSchema = useMemo(
    () => ({
      items: [
        {
          field: "code",
          label: t.pages.platformConfigs?.code || "分组编码",
          type: "text",
          required: true,
        ...
        {
          field: "name",
          label: t.pages.platformConfigs?.name || "分组名称",
          type: "text",
          required: true,
          placeholder: "如：系统配置",
          rules: [
        ...
        {
          field: "description",
          label: t.pages.platformConfigs?.description || "描述",
          type: "textarea",
        ...
        {
          field: "sort",
          label: t.pages.platformConfigs?.sort || "排序",
          type: "number",
          defaultValue: 0,
        },
        {
          field: "is_public",
          label: t.pages.platformConfigs?.isPublic || "公开",
          type: "switch",
          defaultValue: false,
        },
      ],
  ...
  const handleDeleteItem = async () => {
    ...
    try {
      await configApi.deletePlatformItem(
        selectedGroupId,
        deletedId
      )
  ...
  return (
    <PageLayout>
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <h1 className="text-2xl font-semibold tracking-tight">
                {t.pages.platformConfigs?.title || "平台配置"}
              </h1>
              <p className="text-sm text-muted-foreground">
                {t.pages.platformConfigs?.subtitle ||
                  "维护 scope=platform 的配置分组与配置项，供所有租户消费"}
              </p>
            </div>
          </div>
        </div>
            ...
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                <CardTitle className="flex items-center gap-2 text-base">
                  ...
                  {t.pages.platformConfigs?.groupList || "分组列表"}
                </CardTitle>
                <div className="flex gap-1">
                  ...
                  <Button
                    ...
                    title={t.pages.platformConfigs?.addGroup || "新建分组"}
                  >
              ...
              <div className="relative pt-1">
                ...
                <Input
                  placeholder={
                    t.pages.platformConfigs?.searchPlaceholder ||
                    "搜索分组编码或名称..."
                  }
                  value={searchTerm}
              ...
              <div className="flex items-center justify-between">
                <div>
                  <CardTitle className="flex items-center gap-2 text-base">
                    {selectedGroup ? (
                      ...
                      <>
                        <ListIcon className="h-4 w-4" />
                        {t.pages.platformConfigs?.items || "配置项"}
                      </>
                    )}
                  </CardTitle>
                  <CardDescription>
                    {selectedGroup
                      ? `${t.pages.platformConfigs?.items || "配置项"}（共 ${items.length} 项）`
                      : t.pages.platformConfigs?.selectGroupFirst ||
                          "请先在左侧选择一个分组"}
                  </CardDescription>
                </div>
                <div className="flex gap-2">
                  {selectedGroup && (
                    <>
                      ...
                      >
                        ...
                        {t.pages.platformConfigs?.editGroup || "编辑分组"}
                      </Button>
                      <Button
                        variant="outline"
                        size="sm"
                      ...
                      >
                        ...
                        {t.pages.platformConfigs?.deleteGroup ||
                          "删除分组"}
                      </Button>
                    </>
                  )}
                  ...
                  >
                    ...
                    {t.pages.platformConfigs?.addItem || "新增配置项"}
                  </Button>
                </div>
              </div>
            </CardHeader>
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow>
                        ...
                        <TableHead>{t.pages.platformConfigs?.label || "名称"}</TableHead>
                        <TableHead className="w-20">
                          {t.pages.platformConfigs?.type || "类型"}
                        </TableHead>
                        <TableHead>
                          {t.pages.platformConfigs?.value || "当前值"}
                        </TableHead>
                        <TableHead className="w-20">
                          {t.pages.platformConfigs?.sort || "排序"}
                        </TableHead>
      ...
      <FormDialog
        ...
        title={
          groupDialogMode === "add"
            ? t.pages.platformConfigs?.addGroup || "新建分组"
            : t.pages.platformConfigs?.editGroup || "编辑分组"
        }
        width={520}
      ...
      <Dialog open={deleteGroupOpen} onOpenChange={setDeleteGroupOpen}>
        <DialogContent className="sm:max-w-[420px]">
          <DialogHeader>
            <DialogTitle>
              {t.pages.platformConfigs?.deleteGroup || "删除分组"}
            </DialogTitle>
            <DialogDescription>
      ...
      <Dialog open={deleteItemOpen} onOpenChange={setDeleteItemOpen}>
        <DialogContent className="sm:max-w-[420px]">
          <DialogHeader>
            <DialogTitle>
              {t.pages.platformConfigs?.deleteItem || "删除配置项"}
            </DialogTitle>
            <DialogDescription>
    ...
    if (mode === "edit" && item) {
      setForm({
        ...
        optionsText: item.options
          ? safeJsonStringify(item.options)
          : "",
      })
    } else {
      ...
      <DialogContent className="sm:max-w-[640px] max-h-[88vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>
            {mode === "add"
              ? t.pages.platformConfigs?.addItem || "新增配置项"
              : t.pages.platformConfigs?.editItem || "编辑配置项"}
          </DialogTitle>
          <DialogDescription>
        ...
        <form onSubmit={submit} className="space-y-4">
          {error && (
            <div className="text-sm text-destructive border border-destructive/40 bg-destructive/5 rounded-md p-2">
              {error}
            </div>
          )}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <label className="text-sm font-medium">
                {t.pages.platformConfigs?.label || "名称"}
              </label>
              <Input
                value={form.label}
                onChange={(e) =>
          ...
          <div className="space-y-1.5">
            <label className="text-sm font-medium">
              {t.pages.platformConfigs?.description || "描述"}
            </label>
            <Textarea
          ...
          <ValueEditor
            label={t.pages.platformConfigs?.value || "当前值"}
            type={form.type}
            value={form.value}
            onChange={(v) => setForm((f) => ({ ...f, value: v }))}
          />
          <ValueEditor
            label={t.pages.platformConfigs?.defaultValue || "默认值"}
            type={form.type}
            value={form.defaultValue}
            onChange={(v) =>
              setForm((f) => ({ ...f, defaultValue: v }))
            }
          />
          ...
          {(form.type === "select" || form.type === "multiselect") && (
            <div className="space-y-1.5">
              <label className="text-sm font-medium">
                {t.pages.platformConfigs?.options || "可选值（JSON 数组）"}
              </label>
              <Textarea
              ...
              <p className="text-xs text-muted-foreground">
                格式：[{"{"}label, value{"}"}]，value 为 string | number | boolean
              </p>
            </div>
          )}
          ...
          <div className="flex flex-wrap gap-6 pt-1">
            <label className="flex items-center gap-2 text-sm">
              ...
              />
              {t.pages.platformConfigs?.isPublic || "公开"}
            </label>
            <label className="flex items-center gap-2 text-sm">
              <Switch
              ...
              />
              {t.pages.platformConfigs?.isReadonly || "只读"}
            </label>
            <label className="flex items-center gap-2 text-sm">
              <Switch
                ...
                disabled={mode === "edit"}
              />
              {t.pages.platformConfigs?.isSystem || "系统"}
            </label>
          </div>
