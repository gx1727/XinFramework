...
export function AvatarCategoriesPage() {
  ...
  const handleOpenEdit = (cat: AvatarCategoryItem) => {
    setFormData({
      id: cat.id,
      code: cat.code,
      name: cat.name,
  ...
  const handleDelete = (cat: AvatarCategoryItem) => {
    setDeleteTarget(cat)
  }
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (formData.id) {
        ...
        await avatarCategoryApi.update(formData.id, payload)
        setCategories((prev) =>
        ...
        const res = await avatarCategoryApi.create(payload)
        const created = res as AvatarCategoryItem
        if (created?.id) {
      ...
      <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
        <SheetContent side="right" className="w-full sm:max-w-lg overflow-y-auto">
          ...
          <div className="flex flex-col gap-4 px-4">
            ...
            <div className="flex flex-col gap-2">
              <Label htmlFor="cat-icon">图标 URL</Label>
              <Input
                id="cat-icon"
