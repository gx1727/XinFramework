...
export function SysDictsPage() {
  ...
  const handleDeleteDict = async () => {
    ...
    try {
      ...
      const msg = e instanceof Error ? e.message : String(e)
      console.warn("[delete sys dict] failed:", e)
      setError(`删除字典失败：${msg}`)
  ...
  const dictFormSchema: FormSchema = useMemo(
    () => ({
      items: [
        {
          field: "code",
          label: t.pages.sysDicts?.code || "字典编码",
          type: "text",
          required: true,
        ...
        {
          field: "name",
          label: t.pages.sysDicts?.name || "字典名称",
          type: "text",
          required: true,
          placeholder: "如：性别",
        ...
        {
          field: "sort",
          label: t.pages.sysDicts?.sort || "排序",
          type: "number",
          defaultValue: 0,
        },
        {
          field: "visibility",
          label: t.pages.sysDicts?.visibility || "可见性",
          type: "select",
          defaultValue: "all",
          options: [
            { label: t.pages.sysDicts?.visibilityAll || "全部", value: "all" },
            { label: t.pages.sysDicts?.visibilityWhitelist || "白名单", value: "whitelist" },
            { label: t.pages.sysDicts?.visibilityBlacklist || "黑名单", value: "blacklist" },
          ],
        },
      ],
    }),
  ...
  const handleDeleteItem = async () => {
    ...
    try {
      ...
      const msg = e instanceof Error ? e.message : String(e)
      console.warn("[delete sys dict item] failed:", e)
  ...
  const itemFormSchema: FormSchema = useMemo(
    () => ({
      items: [
        {
          field: "code",
          label: t.pages.sysDicts?.itemCode || "字典项编码",
          type: "text",
          required: true,
        ...
        {
          field: "name",
          label: t.pages.sysDicts?.itemName || "字典项名称",
          type: "text",
          required: true,
          placeholder: "如：男",
  ...
  return (
    <PageLayout>
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <h1 className="text-2xl font-semibold tracking-tight">
                {t.pages.sysDicts?.title || "Sys 字典"}
              </h1>
              <p className="text-sm text-muted-foreground">
                {t.pages.sysDicts?.subtitle ||
                  "维护 scope=sys 的数据字典与字典项，供所有租户消费"}
              </p>
            </div>
          </div>
        </div>
            ...
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                <CardTitle className="flex items-center gap-2 text-base">
                  ...
                  {t.pages.sysDicts?.dictList || "字典列表"}
                </CardTitle>
              ...
              <div className="relative pt-1">
                ...
                <Input
                  placeholder={t.pages.sysDicts?.searchPlaceholder || "搜索字典编码或名称..."}
                  value={searchTerm}
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>
                    {selectedDict
                      ? `共 ${items.length} 项`
                      : (t.pages.sysDicts?.selectDictFirst || "请先在左侧选择一个字典")}
                  </CardDescription>
                </div>
                <div className="flex gap-2">
                  ...
                  <Button size="sm" onClick={handleAddItem} disabled={!selectedId}>
                    ...
                    {t.pages.sysDicts?.addItem || "新增字典项"}
                  </Button>
                </div>
              </div>
            </CardHeader>
                        ...
                        <TableCell>
                          {it.status === 1 ? (
                            <Badge variant="default" className="text-[10px]">
                              ...
                              {t.pages.sysDicts?.enabled || "启用"}
                            </Badge>
                          ) : (
                            <Badge variant="secondary" className="text-[10px]">
                              {t.pages.sysDicts?.disabled || "停用"}
                            </Badge>
                          )}
                        </TableCell>
      ...
      <FormDialog
        ...
        title={
          dictDialogMode === "add"
            ? t.pages.sysDicts?.addDict || "新建字典"
            : t.pages.sysDicts?.editDict || "编辑字典"
        }
        width={520}
      ...
      <FormDialog
        ...
        title={
          itemDialogMode === "add"
            ? t.pages.sysDicts?.addItem || "新增字典项"
            : t.pages.sysDicts?.editItem || "编辑字典项"
        }
        width={480}
