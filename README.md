...
- **后端**（[server/](file:///d:\work\xin\XinFramework\server)）：Go 1.25 + Gin + pgx + PostgreSQL，
  - 框架核心：`gx1727.com/xin/framework`
  - 业务模块：`gx1727.com/xin/apps`（按 [Phase 2/3 重构](file:///d:\work\xin\XinFramework\server\doc\architecture.md) 把业务模块搬出 framework 内部）
...
│   │   ├── boot/{auth,tenant}/
│   │   ├── rbac/{user,role,menu,resource,permission,organization}/  # Phase 3 待迁移
│   │   ├── reference/{dict,asset,weixin}/                          # Phase 3 待迁移
│   │   ├── cms/
│   │   └── flag/
│   ├── config/             # 配置
│   ├── migrations/         # SQL 迁移
...
- **插件化模块**：内置模块与外部 app 走同一注册路径（[Phase 1 重构后](file:///d:\work\xin\XinFramework\server\doc\architecture.md#phase-1统一注册)）
...
| 文件 | 内容 |
| --- | --- |
| [architecture.md](file:///d:\work\xin\XinFramework\server\doc\architecture.md) | 架构总览、Go module 切分、Phase 1-5 重构方案 |
