...
CREATE INDEX idx_db_logs_tenant ON db_logs (tenant_id) WHERE is_deleted = FALSE;
CREATE INDEX idx_db_logs_user ON db_logs (user_id) WHERE is_deleted = FALSE;
...
CREATE UNIQUE INDEX uk_account_role ON account_roles (account_id, role) WHERE is_deleted = FALSE;
...
CREATE POLICY tenant_isolation_policy ON users USING (tenant_id = NULLIF(current_setting('app.tenant_id', true), '')::BIGINT);

-- roles
DROP POLICY IF EXISTS tenant_isolation_policy ON roles;
CREATE POLICY tenant_isolation_policy ON roles USING (tenant_id = NULLIF(current_setting('app.tenant_id', true), '')::BIGINT);

-- permissions
DROP POLICY IF EXISTS tenant_isolation_policy ON permissions;
CREATE POLICY tenant_isolation_policy ON permissions USING (tenant_id = NULLIF(current_setting('app.tenant_id', true), '')::BIGINT);

-- role_data_scopes
DROP POLICY IF EXISTS tenant_isolation_policy ON role_data_scopes;
CREATE POLICY tenant_isolation_policy ON role_data_scopes USING (tenant_id = NULLIF(current_setting('app.tenant_id', true), '')::BIGINT);

-- user_roles
DROP POLICY IF EXISTS tenant_isolation_policy ON user_roles;
CREATE POLICY tenant_isolation_policy ON user_roles USING (tenant_id = NULLIF(current_setting('app.tenant_id', true), '')::BIGINT);

-- organizations
DROP POLICY IF EXISTS tenant_isolation_policy ON organizations;
CREATE POLICY tenant_isolation_policy ON organizations USING (tenant_id = NULLIF(current_setting('app.tenant_id', true), '')::BIGINT);

-- tenant_user_seq
DROP POLICY IF EXISTS tenant_isolation_policy ON tenant_user_seq;
CREATE POLICY tenant_isolation_policy ON tenant_user_seq USING (tenant_id = NULLIF(current_setting('app.tenant_id', true), '')::BIGINT);

-- role_menus
DROP POLICY IF EXISTS tenant_isolation_policy ON role_menus;
CREATE POLICY tenant_isolation_policy ON role_menus USING (tenant_id = NULLIF(current_setting('app.tenant_id', true), '')::BIGINT);

-- role_resources
DROP POLICY IF EXISTS tenant_isolation_policy ON role_resources;
CREATE POLICY tenant_isolation_policy ON role_resources USING (tenant_id = NULLIF(current_setting('app.tenant_id', true), '')::BIGINT);
...
-- 租户
INSERT INTO tenants (code, name, status, created_by, updated_by) VALUES ('default', '默认租户', 1, 0, 0);
...
-- 用户角色
INSERT INTO user_roles (tenant_id, user_id, role_id) VALUES (1, 1, 1);
...
SELECT 1, 1, id FROM menus WHERE is_deleted = FALSE;
